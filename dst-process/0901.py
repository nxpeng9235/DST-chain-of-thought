                ...
                for domain in schema:
                    ...
                    for slot in slots:
                        ...
                        if slot["name"] in active_slot_values.keys():
                            ...
                            ## placing the answer at the end fo the sentence.
                            chain_of_thought = "Because "
                            tmp = []
                            for ut in related_uttrs:
                                thought = f'the system said "{ut[0]}", and ' if ut[0] != "###NULL###" else ""
                                thought += f'the user said "{ut[1]}"'
                                tmp.append(thought)
...
def get_predicted_slot_value(output_sent):
    return output_sent.strip(".").split(" is ")[-1].strip()
