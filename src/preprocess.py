            ...
            if turn["speaker"] == "USER":
                ...
                for frame_idx in range(len(turn["frames"])):
                    ...
                    for key, values in frame["state"]["slot_values"].items():
                        # if have multiple answers, randomly select one
                ...
                for domain in schema:
                    ...
                    for slot in slots:
                        ...
                        if slot["name"] in active_slot_values.keys():
                            ...
                            for ut in uttr_list:
                                if ut.lower().find(target_value) >= 0:
                                    related_uttrs.append(ut)
                            chain_of_thought = "Because "
                            chain_of_thought += ", and ".join([f'"{ut}"'])
                            target_value = f"{chain_of_thought}, the {d_name} {s_name} is {target_value}."
                        else:
                            target_value = "NONE"
